...
function Insurances() {
    ...
    const LabelCus = ({title,id}) => {
        ...
        return(
        <div key={id} className={cx('ranges',id == selectedRange ? 'selected' : '','rounded-2xl border-2 mr-2 ml-2')} id={id}
            onClick={handleClick}>
           {title}
        </div>
        )
        ...
        return(
            <div className = {cx('product')}>
                    ...
                    <div style={{justifyContent: 'center', alignItems: 'center', display: 'flex'}}>
                        <Button style={{backgroundColor: '#3e8bcc'}} className='text-center mt-4 w-full'>Mua ngay</Button>
                    </div>
    ...
    return(
        <div className='bg-white'>
            ...
            <div className='supplemental_benefits'>
                ...
                <div className="flex items-center space-x-2 inline-flex mr-12">
                    <Checkbox id="pregnant" onClick={() => handleProfit()} className='data-[state=checked]:bg-blue-500  dark:data-[state=checked]:bg-blue-100 dark:data-[state=checked]:text-blue-800'/>
                    <Label htmlFor="pregnant" className ='font-normal'>Thai sản</Label>
                </div>
                <div className="flex items-center space-x-2 inline-flex mr-12">
                    <Checkbox id="outpatient" onClick={() => handleProfit()} className='data-[state=checked]:bg-blue-500  dark:data-[state=checked]:bg-blue-100 dark:data-[state=checked]:text-blue-800'/>
                    <Label htmlFor="outpatient" className ='font-normal'>Bảo hiểm ngoại trú</Label>
                </div>
                <div className="flex items-center space-x-2 inline-flex mr-12">
                    <Checkbox id="dentistry" onClick={() => handleProfit()} className='data-[state=checked]:bg-blue-500  dark:data-[state=checked]:bg-blue-100 dark:data-[state=checked]:text-blue-800'/>
                    <Label htmlFor="dentistry" className ='font-normal'>Nha khoa</Label>
                </div>
                <div className="flex items-center space-x-2 inline-flex mr-12">
                    <Checkbox id="inpatient" onClick={() => handleProfit()} className='data-[state=checked]:bg-blue-500  dark:data-[state=checked]:bg-blue-100 dark:data-[state=checked]:text-blue-800'/>
                    <Label htmlFor="inpatient" className ='font-normal'>Trợ cấp nằm viện</Label>
                </div>
            </div>
            <div className='insurance grid grid-cols-5 gap-6 sm:gap-4 mt-10 pb-20'>
                {packageArray}
            </div> 
        </div>
    );
